import { useQuery } from '@tanstack/react-query';
import { createContext, useEffect, useState } from 'react';
import type { LoginResponse } from '@/features/auth/types/auth';
import type { User } from '@/shared/types/user';
import { LoadingSpinner } from '@/shared/components/loading';
import { tokenService } from '@/shared/services/api';
import { attemptLogout, attemptRefreshToken } from '@/features/auth/services/auth';
import { userQueryKeys } from '@/shared/services/query-keys';
import { getMe } from '@/features/auth/services/users';

type AuthStatus = 'loading' | 'authenticated' | 'unauthenticated' | 'error';

interface AuthContextData {
  authenticate: (loginResponse: LoginResponse) => void;
  isAuthenticated: boolean;
  logout: () => void;
  user?: User;
  authStatus: AuthStatus;
}

export const AuthContext = createContext<AuthContextData>({} as AuthContextData);

export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  const [authStatus, setAuthStatus] = useState<AuthStatus>('loading');
  const isAuthenticated = authStatus === 'authenticated';
  const { data } = useQuery({
    queryKey: userQueryKeys.me,
    queryFn: getMe,
    enabled: authStatus === 'authenticated',
  });

  const initAuth = async () => {
    try {
      const refreshTokenResponse = await attemptRefreshToken();
      if (refreshTokenResponse.access_token) {
        tokenService.setToken(refreshTokenResponse.access_token);
        setAuthStatus('authenticated');
      } else {
        setAuthStatus('unauthenticated');
      }
    } catch (error) {
      setAuthStatus('unauthenticated');
    }
  };

  useEffect(() => {
    initAuth();
  }, []);

  const authenticate = (loginResponse: LoginResponse) => {
    tokenService.setToken(loginResponse.access_token);
    setAuthStatus('authenticated');
  };

  const logout = async () => {
    try {
      await attemptLogout();
      setAuthStatus('unauthenticated');
      tokenService.setToken('');
    } catch (error) {
      setAuthStatus('unauthenticated');
      tokenService.setToken('');
    } finally {
    }
  };

  if (authStatus === 'loading') {
    return (
      <main className="from-muted to-muted flex min-h-screen items-center justify-center bg-gradient-to-br p-4">
        <LoadingSpinner size="4em" />
      </main>
    );
  }

  return (
    <AuthContext.Provider value={{ authenticate, isAuthenticated, logout, user: data, authStatus }}>
      {children}
    </AuthContext.Provider>
  );
};
