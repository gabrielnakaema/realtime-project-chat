...
func TestUserService_Create(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mockSetup: func(repo *mockUserRepository) {
				repo.On("Create", mock.Anything, mock.AnythingOfType("*domain.User")).Return(apperr.DuplicateEntryError("user email is already taken"))
			},
			expectedError: apperr.DuplicateEntryError("user email is already taken"),
			shouldSucceed: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := NewUserService(mockJWT, mockRepo)
...
func TestUserService_Login(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mockUserSetup: func(repo *mockUserRepository) {
				repo.On("GetByEmail", mock.Anything, "nonexistent@example.com").Return(nil, apperr.NotFoundError("user not found"))
			},
			mockJWTSetup:  func(jwt *mockJWTProvider) {},
			shouldSucceed: false,
	...
	}{
		{
			...
			mockSetup: func(repo *mockUserRepository) {
				...
				repo.On("UpdatePassword", mock.Anything, mock.AnythingOfType("*domain.User")).Return(nil).Run(func(args mock.Arguments) {
					...
					matches, compareErr := CompareHash("newpassword123", updatedUser.Password)
...
func TestUserService_RefreshToken(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mockUserSetup: func(repo *mockUserRepository) {
				repo.On("GetRefreshToken", mock.Anything, "invalid-token").Return(nil, apperr.NotFoundError("refresh token not found"))
			},
			mockJWTSetup:  func(jwt *mockJWTProvider) {},
			shouldSucceed: false,
		},
...
func TestCompareHash(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := CompareHash(tt.password, tt.hash)
...
func TestGenerateRefreshToken(t *testing.T) {
	...
	t.Run("generates unique tokens", func(t *testing.T) {
		token1, _ := GenerateRefreshToken(32)
		token2, _ := GenerateRefreshToken(32)
