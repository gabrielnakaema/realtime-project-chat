...
func (par *ProjectActivityRepository) List(ctx context.Context, params ListProjectActivityLogsParams) (*utils.CursorPaginated[domain.ProjectActivity], error) {
	...
	results, err := q.ListProjectActivityLogs(ctx, projectqueries.ListProjectActivityLogsParams{
	...
	for i, result := range results {
		...
		if activity.EntityType == domain.TaskEntityType && len(result.Entity) > 0 {
			...
			if err != nil {
				return nil, apperr.ServerError("failed to unmarshal task", err)
			}
			activity.Task = &task
		}
		...
		if activity.EntityType == domain.ProjectMemberEntityType {
			if len(result.Entity) > 0 {
				...
				if err != nil {
					return nil, apperr.ServerError("failed to unmarshal project member", err)
				}
