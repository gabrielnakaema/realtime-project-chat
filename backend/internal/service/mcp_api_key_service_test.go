...
func TestMCPAPIKeyService_CreateNormalizesNameAndScopes(t *testing.T) {
	...
	svc := service.NewMCPAPIKeyService(repo)
	userID := uuid.New()
	...
	result, err := svc.Create(context.Background(), service.CreateMCPAPIKeyRequest{
		UserID: userID,
		Name:   "  Claude Desktop  ",
...
func TestMCPAPIKeyService_Update(t *testing.T) {
	t.Run("normalizes replacement fields and returns reloaded key", func(t *testing.T) {
		...
		result, err := svc.Update(context.Background(), service.UpdateMCPAPIKeyRequest{
			ID:     keyID,
			UserID: userID,
			Name:   "  Updated agent  ",
			Scopes: []domain.MCPAPIScope{
