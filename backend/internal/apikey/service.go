...
func (s *MCPAPIKeyService) Create(ctx context.Context, request CreateMCPAPIKeyRequest) (*CreateMCPAPIKeyResult, error) {
	...
	if err != nil {
		return nil, apperr.ServerError("failed to generate mcp api key", err)
	}
	...
	if err := s.repository.Create(ctx, key); err != nil {
		return nil, apperr.ServerError("failed to create mcp api key", err)
	}
...
func (s *MCPAPIKeyService) Update(ctx context.Context, request UpdateMCPAPIKeyRequest) (*domain.MCPAPIKey, error) {
	...
	if err != nil {
		...
		return nil, apperr.ServerError("failed to get mcp api key", err)
	}

	if key.IsRevoked() {
		return nil, apperr.BusinessValidationError("mcp api key is already revoked")
	}
	...
	if err := s.repository.Update(ctx, key); err != nil {
		...
		return nil, apperr.ServerError("failed to update mcp api key", err)
	}
...
func (s *MCPAPIKeyService) ListByUserID(ctx context.Context, userID uuid.UUID) ([]domain.MCPAPIKey, error) {
	...
	if err != nil {
		return nil, apperr.ServerError("failed to list mcp api keys", err)
	}
...
func (s *MCPAPIKeyService) Revoke(ctx context.Context, id uuid.UUID, userID uuid.UUID) error {
	...
	if err != nil {
		...
		return apperr.ServerError("failed to get mcp api key", err)
	}

	if key.IsRevoked() {
		return apperr.BusinessValidationError("mcp api key is already revoked")
	}
	...
	if err := s.repository.Revoke(ctx, id, userID, time.Now()); err != nil {
		...
		return apperr.ServerError("failed to revoke mcp api key", err)
	}
...
func (s *MCPAPIKeyService) Authenticate(ctx context.Context, bearerSecret string) (*AuthenticateMCPAPIKeyResult, error) {
	...
	if err != nil {
		return nil, apperr.UnauthorizedError("invalid api key")
	}
	...
	if err != nil {
		...
		return nil, apperr.ServerError("failed to load mcp api key", err)
	}
	...
	if key.IsRevoked() {
		return nil, apperr.UnauthorizedError("api key revoked")
	}

	if err := s.repository.TouchLastUsedAt(ctx, key.ID, time.Now(), mcpAPIKeyLastUsedWriteInterval); err != nil {
		return nil, apperr.ServerError("failed to update mcp api key usage", err)
	}
...
func normalizeMCPAPIKeyInput(name string, scopes []domain.MCPAPIScope) (string, []domain.MCPAPIScope, error) {
	...
	if normalizedName == "" {
		return "", nil, apperr.BusinessValidationError("name is required")
	}
...
func normalizeMCPScopes(scopes []domain.MCPAPIScope) ([]domain.MCPAPIScope, error) {
	if len(scopes) == 0 {
		return nil, apperr.BusinessValidationError("at least one scope is required")
	}
	...
	for _, scope := range scopes {
		...
		if !key.IsAllowedScope(scope) {
			return nil, apperr.BusinessValidationError("invalid mcp api key scope")
		}
