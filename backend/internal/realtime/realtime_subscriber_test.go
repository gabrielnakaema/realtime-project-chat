...
func TestRealtimeSubscriberDeliversEventsToWebsocketNotifier(t *testing.T) {
	...
	require.NoError(t, sub.handleEvents(context.Background(), messaging.Message{Topic: events.ChatMessageCreated, Value: messageValue}))
	...
	require.NoError(t, err)
	require.NoError(t, sub.handleEvents(context.Background(), messaging.Message{Topic: events.ChatMemberCreated, Value: memberValue}))
	...
	require.NoError(t, err)
	require.NoError(t, sub.handleEvents(context.Background(), messaging.Message{Topic: events.ChatMessageRead, Value: readValue}))
	require.Equal(t, chatID, notifier.roomID)
	...
	require.NoError(t, err)
	require.NoError(t, sub.handleEvents(context.Background(), messaging.Message{Topic: events.ChatMemberRemoved, Value: removedValue}))
	require.Equal(t, chatID, notifier.roomID)
